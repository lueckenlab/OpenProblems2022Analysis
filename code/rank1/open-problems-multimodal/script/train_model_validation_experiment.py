...
class ValidationSetEvaluation(object):
    def __init__(self):
        pass
    ...
    def compute_score(
        ...
        for validation_strategy in val_strategies:
            print("Validation strategy:", validation_strategy)

            print("Extracting indices")
            idx_tr = np.where(1 - metadata[validation_strategy])[0]
            idx_va = np.where(metadata[validation_strategy])[0]
            
            print("idx_tr", idx_tr)
            print("Training set length", len(idx_tr))
            print("Validation set length", len(idx_va))
            ...
            for bagging_i in range(_n_bagging):
                ...
                corrscore_val = correlation_score(y_val, new_y_val_pred)
                print(
                    f"Fold {validation_strategy} bagging {bagging_i} "
                    f"mse_train: {mse_train: .5f} "
                    f"corrscore_train: {corrscore_train: .5f} "
                    f"mse_val: {mse_val: .5f} "
                    f"corrscore_val: {corrscore_val: .5f} "
            ...
            for cell_type_name in cell_types:
                ...
                if s.sum() > 10:
                    ...
                    score[f"{cell_type_name}_corrscore_val"] = 1.0
            if dump:
                np.save(os.path.join(dump_dir, f"k{validation_strategy}_y_val.npy"), y_val)
                np.save(os.path.join(dump_dir, f"k{validation_strategy}_y_val_pred.npy"), y_val_pred)

            del x_train, y_train, y_train_pred, x_val, y_val_pred, y_val
            print(f"Fold {validation_strategy}: score:{score} elapsed time = {time.time() - start_time: .3f}")
            for k, v in score.items():
                scores_dict[k].append(v)
...
def main():
    ...
    parser.add_argument("--validation_split_path", metavar="PATH")
    ...
    # Read file with validation splits and merge it with metadata
    val_strategies = (
        "adversarial_val",
        "random_third_val",
        "random_fifth_val",
        ...
        "donor_32606_val",
        "donor_31800_val",
        "day_2_donor_32606_val",
        "day_2_donor_31800_val",
        "day_4_donor_32606_val",
        "day_4_donor_31800_val",
    )

    validation_splits = pd.read_csv(args.validation_split_path)
    validation_splits = validation_splits.set_index("cell_id")
    train_metadata[list(val_strategies)] = validation_splits.loc[train_metadata.index, val_strategies]
    ...
    if not args.skip_test_prediction:
        if args.use_k_fold_models:
            ...
            for idx, (model, pre_post_process) in enumerate(zip(k_fold_models, k_fold_pre_post_processes)):
                ...
                new_y_test_pred = row_normalize(
                    pre_post_process.postprocess(preprocessed_y_test_pred)
                )
                if y_test_pred is None:
                    y_test_pred = new_y_test_pred
                else:
                    y_test_pred += new_y_test_pred

                if args.task_type == "multi":
                    pred_file_path = f"multimodal_pred_{val_strategies[idx]}_seed_{args.seed}.pickle"
                elif args.task_type == "cite":
                    pred_file_path = f"citeseq_pred_{val_strategies[idx]}_seed_{args.seed}.pickle"

                with open(os.path.join(args.out_dir, pred_file_path), "wb") as f:
                    pickle.dump(new_y_test_pred, f)

                model_dir = os.path.join(args.out_dir, f"model_{val_strategies[idx]}_seed_{args.seed}")
                os.makedirs(model_dir, exist_ok=True)
                with open(os.path.join(model_dir, f"pre_post_process_seed.pickle"), "wb") as f:
            ...
            os.makedirs(model_dir, exist_ok=True)
            with open(os.path.join(model_dir, f"pre_post_process_seed_{args.seed}.pickle"), "wb") as f:
                pickle.dump(pre_post_process, f)
            model.save(model_dir)
            if args.check_load_model:
                with open(os.path.join(model_dir, f"pre_post_process_seed_{args.seed}.pickle"), "rb") as f:
                    _ = pickle.load(f)
        ...
        print("save results")
        if args.task_type == "multi":
            pred_file_path = f"multimodal_pred_seed_{args.seed}.pickle"
        elif args.task_type == "cite":
            pred_file_path = f"citeseq_pred_seed_{args.seed}.pickle"
        else:
            raise ValueError
        with open(os.path.join(args.out_dir, pred_file_path), "wb") as f:
